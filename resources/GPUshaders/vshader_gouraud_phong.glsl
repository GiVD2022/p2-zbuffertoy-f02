...
void main()
{
    ...
    for (int i=0; i < 5; i++){ //array de lights de length 5
        ...
        if (light.type == 0) // point light
        {
            ...
            lightSpecular += attenuation * mat_info.Ks * light.Is * pow(max(dot(R, V), 0.0f), mat_info.shininess);
        }
        else if (light.type == 1) // spot light
        {
            ...
            if (spotEffect > light.spotCosineCutoff) { //surface point is inside the cone of the spot light
                ...
                R = normalize( 2.0 * (dot(N, L) * N) -L);
                V = normalize(obs - vPosition);
                lightSpecular += attenuation * mat_info.Ks * light.Is * spotEffect * pow(max(dot(R, V), 0.0f), mat_info.shininess);
            }
